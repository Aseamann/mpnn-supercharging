"""Figure builders for notebooks/analysis_updated.ipynb.

PLAN.md Section 11.4, added 2026-09-05. A second module, not a replacement.
`analysis_lib.py` and `analysis.ipynb` are untouched, so the 17 figures verified
in PLAN.md Section 14.1 stay reproducible; this module carries the reworked
F1, F2, F3, F4, F5, F6, F7, F8, F9 and the new F10 over the nine-arm set, and
writes to `figures/updated/`.

Loading, styling and saving are imported from `analysis_lib` rather than copied.
The same Section 11 constraint applies here: reads only `results/*.csv` and
`data/scaffold_manifest.csv`, writes only `figures/`, touches no cluster, and
recomputes no metric that `10_aggregate.py` already wrote.

**Titles carry no context.** In `analysis_lib.py` several titles hold whole
caveat sentences. Here every title is the short form `F2  Mutational efficiency`
and the context lives in a markdown Notes cell under each figure in the
notebook. Nothing was dropped: the caveats that were only in a title are written
out at greater length there.

**Palette.** Nine arms cannot be nine distinct colourblind-safe hues, and the
six-slot palette in `analysis_lib.py` already fails its own all-pairs check. The
palette here groups arms into one hue family per method family, with lightness
...
Reproduce with:
    ...
    import analysis_updated_lib as U; print(','.join(U.PALETTE))")" --pairs adjacent

So the same rule as before holds, with more margin on the colourblind axis than
the six-slot palette had (14.7 against its 9.1) and less on the unsimulated one:
neighbouring series are safe in lines, grouped bars and boxplots; any two series
landing side by side are not, so F7 facets by method instead of overlaying.
Colour is never the only channel. Every multi-series figure carries a legend and
markers differ by method.

Note that `mpnn_soluble` changes colour relative to `analysis_lib.py`, from the
mid blue `#2a78d6` to the dark blue `#12406f`, because the family grouping gives
the primary arm the most saturated slot of its family. Figures from the two
notebooks are therefore not colour-comparable side by side, which is why they
are written to different directories.
"""
...
FIGURE_DIR = A.ROOT / "figures" / "updated"
...
def save(fig, name: str) -> None:
    """PNG and PDF at 300 dpi into figures/updated/, not figures/."""
    FIGURE_DIR.mkdir(parents=True, exist_ok=True)
    for ext in ("png", "pdf"):
        fig.savefig(FIGURE_DIR / f"{name}.{ext}")
    print(f"wrote figures/updated/{name}.png and .pdf")
...
def load_updated() -> dict:
    """`analysis_lib.load_all` plus the unguided-MPNN hit table."""
    data = load_all()
...
def fig1(data: dict):
    ...
    ax.barh(ypos, 2 * 1.96 * dist["q_sd"], left=dist["q_mean"] - 1.96 * dist["q_sd"],
            ...
            label=f"vanilla sample mean ± 1.96 sd ({n_label} samples)")
    ...
    for y, scaffold in zip(ypos, dist["scaffold_id"]):
        ...
        ax.scatter(miss, np.full(len(miss), y), marker="x", s=16,
                   color="#c1272d", linewidths=1.0, zorder=4)
        ax.scatter(hit, np.full(len(hit), y), marker="o", s=13,
                   facecolor="white", edgecolor="#006c31", linewidths=1.0,
                   zorder=4)
    ...
    # colour.
    ax.scatter([], [], marker="x", s=16, color="#c1272d", linewidths=1.0,
               label="ladder target, 0 sampled")
    ax.scatter([], [], marker="o", s=13, facecolor="white", edgecolor="#006c31",
               linewidths=1.0, label="ladder target, ≥ 1 sampled")
...
def fig2(data: dict):
    ...
    axes[-1].legend(handles=handles, frameon=False, fontsize=7.5,
                    loc="center left", bbox_to_anchor=(1.02, 0.5))
    fig.suptitle("F2  Mutational efficiency", y=1.02, fontsize=11)
    fig.tight_layout()
    return fig
...
def fig6(data: dict):
    ...
    ax.set_title(f"h-bond protection on (dotted) vs off (solid), "
                 f"{len(shared)} shared scaffolds", fontsize=9, loc="left")
    ...
    fig.suptitle("F6  Hydrogen bonds gained and lost", y=1.03, fontsize=11)
    fig.tight_layout()
    return fig
...
def fig7(data: dict):
    ...
    for ax in axes[-1]:
        ax.set_xlabel("ΔQ density", fontsize=8)
...
def fig9(data: dict):
    ...
    ax.set_xticklabels(MUT_BIN_LABELS_SHOWN, rotation=45, ha="right", fontsize=7)
...
def fig10(data: dict):
    ...
    ax.set_title("a  temperature the target cost", fontsize=9, loc="left")
    ...
    fig.suptitle("F10  Charge demand, decoding temperature, predicted "
                 "confidence (MPNN arms)", y=1.02, fontsize=11)
    fig.tight_layout()
    return fig
...
def temperature_confound(data: dict) -> pd.DataFrame:
    ...
    """
    d = temperature_frame(data)
    out = d.groupby("final_temperature").agg(
